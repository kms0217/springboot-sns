...
public class ProfileEditValidator implements Validator {
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (userEmail == null && validEmail != null) {
        ...
        if (userPhone == null && validPhone != null) {
        ...
        // 4. 중복된 User가 있는지 확인
        if (userUsername != validUsername) {
            if (validUsername == null) {
                errors.rejectValue("username", "key", "사용자 이름을 입력해주세요.");
            } else if (authService.getUserByUserName(validUsername).isPresent()){
